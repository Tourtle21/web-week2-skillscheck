...
const lovesCodeChecker = x => x;//Code here
...
const oddChecker = num => num % 2 === 0 ? 'the number is even' : 'the number is odd';//Code here
...
const iLove = (name, love) => `${name} loves ${love}`;//Code Here
...
let middleNums = numbers.slice(1, numbers.length - 1);//Code Here
...
// const bigOrSmall = arr => arr.map(num => num > 100 ? 'big' : 'small');// Code here
...
//const arrayReverser = arr => arr.sort((a, b) => b - a);//Code Here
...
const total = myNumbers.reduce((acc, num) => acc+num);// Code Here
...
let myNumbersIndex = [];
myNumbers.forEach((num, i) => myNumbersIndex.push(i));// Code Here
...
const forTheLoveOfGeorge = notGeorge.map(name => "George");// Code Here
...
const enemies = people.filter(person => !person.friend);// Code Here
...
let totallyAwesome = people.reduce((acc, person) => acc + person.awesomeLevel, 0);// Code Here
